...
test('usageTotals sums rows for current user', async () => {
  const user = await dataSource.getRepository(User).save({ email: 'u1@example.com', password: 'x', permissions: [] });
  ...
  assert.deepEqual(out, { totalRequests: 100, totalErrors: 5, totalRateLimited: 2 });
});

test('usageDailyRequests maps series', async () => {
  const user = await dataSource.getRepository(User).save({ email: 'u2@example.com', password: 'x', permissions: [] });
  const app = await dataSource.getRepository(Application).save({ name: 'app2', ownerId: user.id });
  ...
  await dataSource.getRepository(ApplicationUsage).save([
    { applicationId: app.id, serviceId: s1.id, date: '2025-09-21', requestCount: 10, errorCount: 0, rateLimitExceededCount: 0 },
    { applicationId: app.id, serviceId: s2.id, date: '2025-09-22', requestCount: 25, errorCount: 0, rateLimitExceededCount: 0 }
  ]);
  const resolver = new UsageDashboardResolver();
...
test('usageTopApiKeys maps rows', async () => {
  const user = await dataSource.getRepository(User).save({ email: 'u3@example.com', password: 'x', permissions: [] });
  const app = await dataSource.getRepository(Application).save({ name: 'app3', ownerId: user.id });
