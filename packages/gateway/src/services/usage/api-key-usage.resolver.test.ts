...
describe.skip('ApiKeyUsageResolver', () => {
  before(async () => {
  ...
  test('apiKeyUsage denies non-owner non-admin', async () => {
    ...
    const owner = await dataSource
      .getRepository(User)
      .save({ email: 'o@example.com', password: 'x', permissions: [] });
    const other = await dataSource
      .getRepository(User)
      .save({ email: 'x@example.com', password: 'x', permissions: [] });
    const app = await dataSource
      .getRepository(Application)
      .save({ name: 'app', ownerId: owner.id });
