...
describeWithDatabase('ApplicationResolver', () => {
  ...
  beforeEach(async () => {
    ...
    resolver = new ApplicationResolver(applicationRepository, serviceRepository, apiKeyService);

    // Users
    owner = await userRepository.save(
      userRepository.create({ email: 'owner@app.test', password: 'pwd', permissions: ['user'] })
    );
    otherUser = await userRepository.save(
      userRepository.create({ email: 'other@app.test', password: 'pwd', permissions: ['user'] })
    );
    admin = await userRepository.save(
      userRepository.create({ email: 'admin@app.test', password: 'pwd', permissions: ['user', 'admin'] })
    );

    // Applications
    appOwned = await applicationRepository.save(
      applicationRepository.create({ name: 'Owner App', description: 'Test', ownerId: owner.id })
    );
    appOther = await applicationRepository.save(
      applicationRepository.create({ name: 'Other App', description: 'Test', ownerId: otherUser.id })
    );
  ...
  describe('queries', () => {
    it('myApplications returns only apps owned by user', async () => {
      const ctx: MockContext = { user: { id: owner.id, permissions: ['user'] } };
  ...
  describe('createApplication', () => {
    it('creates new app with current user as owner', async () => {
      const ctx: MockContext = { user: { id: owner.id, permissions: ['user'] } };
      const app = await resolver.createApplication('New App', 'desc', ctx as any);
      assert.ok(app.id);
      assert.strictEqual(app.ownerId, owner.id);
      const found = await applicationRepository.findOne({ where: { id: app.id } });
      assert.ok(found);
    });
  });
  ...
  describe('service whitelisting', () => {
    it('adds accessible service to app (owner)', async () => {
      const ctx: MockContext = { user: { id: owner.id, permissions: ['user'] } };
      const ok = await resolver.addServiceToApplication(appOwned.id, svcAccessible.id, ctx as any);
      assert.strictEqual(ok, true);
      const app = await applicationRepository.findOne({ where: { id: appOwned.id }, relations: ['whitelistedServices'] });
      assert.ok(app);
      assert.strictEqual(app.whitelistedServices.length, 1);
      assert.strictEqual(app.whitelistedServices[0].id, svcAccessible.id);

      // Idempotent
      const ok2 = await resolver.addServiceToApplication(appOwned.id, svcAccessible.id, ctx as any);
      assert.strictEqual(ok2, true);
      const app2 = await applicationRepository.findOne({ where: { id: appOwned.id }, relations: ['whitelistedServices'] });
      assert.ok(app2);
      assert.strictEqual(app2.whitelistedServices.length, 1);
    });

    it('rejects adding by non-owner non-admin', async () => {
      const ctx: MockContext = { user: { id: otherUser.id, permissions: ['user'] } };
      await assert.rejects(
        () => resolver.addServiceToApplication(appOwned.id, svcAccessible.id, ctx as any),
    ...
    it('rejects when service not externally accessible or inactive', async () => {
      const ctx: MockContext = { user: { id: owner.id, permissions: ['user'] } };
      await assert.rejects(
        () => resolver.addServiceToApplication(appOwned.id, svcInactive.id, ctx as any),
        /Service not found or not externally accessible/
      );
      await assert.rejects(
        () => resolver.addServiceToApplication(appOwned.id, svcNotExternal.id, ctx as any),
        /Service not found or not externally accessible/
      );
      await assert.rejects(
        () => resolver.addServiceToApplication(appOwned.id, '00000000-0000-0000-0000-000000000000', ctx as any),
        /Service not found or not externally accessible/
      );
    });
    ...
    it('rejects for non-existent application', async () => {
      const ctx: MockContext = { user: { id: owner.id, permissions: ['user'] } };
      await assert.rejects(
        () => resolver.addServiceToApplication('00000000-0000-0000-0000-000000000000', svcAccessible.id, ctx as any),
    ...
    it('removes service from application (owner)', async () => {
      const ctx: MockContext = { user: { id: owner.id, permissions: ['user'] } };
      await resolver.addServiceToApplication(appOwned.id, svcAccessible.id, ctx as any);
      const ok = await resolver.removeServiceFromApplication(appOwned.id, svcAccessible.id, ctx as any);
      assert.strictEqual(ok, true);
      const app = await applicationRepository.findOne({ where: { id: appOwned.id }, relations: ['whitelistedServices'] });
      assert.ok(app);
    ...
    it('rejects removal by non-owner non-admin', async () => {
      const ctx: MockContext = { user: { id: otherUser.id, permissions: ['user'] } };
      await assert.rejects(
        () => resolver.removeServiceFromApplication(appOwned.id, svcAccessible.id, ctx as any),
        /Insufficient permissions/
      );
    });
  });
  ...
  describe('API keys', () => {
    it('creates an API key for application (owner) and persists it', async () => {
      const ctx: MockContext = { user: { id: owner.id, permissions: ['user'] } };
      const key = await resolver.createApiKey(appOwned.id, 'primary', ['read'], undefined as any, ctx as any);
      assert.ok(typeof key === 'string' && key.startsWith('app_'));
      const keys = await apiKeyRepository.find({ where: { applicationId: appOwned.id } });
    ...
    it('rejects API key creation by non-owner non-admin', async () => {
      const ctx: MockContext = { user: { id: otherUser.id, permissions: ['user'] } };
      await assert.rejects(
        () => resolver.createApiKey(appOwned.id, 'bad', [], undefined as any, ctx as any),
        /Insufficient permissions/
      );
    });
    ...
    it('revokes API key by owner and by admin', async () => {
      ...
      const { entity } = await apiKeyService.generateApiKey(appOwned.id, 'to-revoke');
      const ownerCtx: MockContext = { user: { id: owner.id, permissions: ['user'] } };
      const adminCtx: MockContext = { user: { id: admin.id, permissions: ['user', 'admin'] } };

      // Owner can revoke
      const ok = await resolver.revokeApiKey(entity.id, ownerCtx as any);
      assert.strictEqual(ok, true);
      let updated = await apiKeyRepository.findOne({ where: { id: entity.id } });
      assert.ok(updated);
      ...
      const otherApp = await applicationRepository.save(
        applicationRepository.create({ name: 'Other User App', ownerId: otherUser.id })
      );
      const { entity: entity2 } = await apiKeyService.generateApiKey(otherApp.id, 'to-revoke-2');
  ...
  describe('listing relations', () => {
    it('getApplicationAccessibleServices returns whitelisted services for owner', async () => {
      const ctx: MockContext = { user: { id: owner.id, permissions: ['user'] } };
      await resolver.addServiceToApplication(appOwned.id, svcAccessible.id, ctx as any);
      const services = await resolver.getApplicationAccessibleServices(appOwned.id, ctx as any);
    ...
    it('getApplicationApiKeys returns keys for owner', async () => {
      const { entity: k1 } = await apiKeyService.generateApiKey(appOwned.id, 'k1');
      const { entity: k2 } = await apiKeyService.generateApiKey(appOwned.id, 'k2');
      assert.ok(k1 && k2);
      const ctx: MockContext = { user: { id: owner.id, permissions: ['user'] } };
      const keys = await resolver.getApplicationApiKeys(appOwned.id, ctx as any);
    ...
    it('getApplicationApiKeys rejects non-owner non-admin', async () => {
      const ctx: MockContext = { user: { id: otherUser.id, permissions: ['user'] } };
      await assert.rejects(() => resolver.getApplicationApiKeys(appOwned.id, ctx as any), /Insufficient permissions/);
    });
  });
});
