...
describe.skip('AuditLog retention cleanup', () => {
  before(async () => {
  ...
  test('should delete only expired records', async () => {
    ...
    assert.equal(
      deleted,
      2,
      'Should delete exactly the two expired records inserted by this test'
    );
    ...
    assert.equal(
      reloadedPast2,
      null,
      'Second expired record should be deleted'
    );
    ...
    assert.ok(
      reloadedFuture!.retentionUntil &&
        reloadedFuture!.retentionUntil > new Date(),
