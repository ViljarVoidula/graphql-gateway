...
describe('API Key Usage Consolidator', () => {
  before(async () => {
  ...
  test('consolidator drains a key and upserts into Postgres', async () => {
    ...
    const user = await dataSource
      .getRepository(User)
      .save({ email: 'owner@example.com', password: 'x', permissions: [] });
    ...
    const prefix =
      process.env.API_KEY_USAGE_REDIS_PREFIX || 'gqlgw:ak:usage:v1';
    ...
    const rows = await dataSource
      .getRepository(ApiKeyUsage)
      .find({ where: { apiKeyId: key.id, date } });
