...
if (!(Reflect as any)?.defineMetadata) {
  throw new Error('reflect-metadata not loaded. Ensure import "reflect-metadata" runs before any decorated classes.');
}
...
async function loadServicesFromDatabase(): Promise<string[]> {
  ...
  try {
    ...
    const services = (await (serviceRegistry as any).getPollableServices?.()) || (await serviceRegistry.getAllServices());
...
const loader = new SchemaLoader(
  function buildSchemaFromEndpoints(loadedEndpoints) {
    const subschemas: Array<GraphQLSchema | any> = loadedEndpoints.map(({ sdl, url, useMsgPack, typePrefix }) => {
      const schema = buildSchema(sdl);
      const subschemaConfig: any = {
        schema,
      ...
      const trimmedPrefix = typePrefix?.trim();
      if (trimmedPrefix) {
        const safePrefix = trimmedPrefix.endsWith('_') ? trimmedPrefix : `${trimmedPrefix}_`;
        subschemaConfig.transforms = [
          new RenameTypes((name: string) => {
            ...
            if (name.startsWith(safePrefix)) return name;
...
try {
  ...
  log.warn('Deferred initial local schema build failed; relying on lazy reload', {
  ...
  schema: async () => {
    ...
    if (!loader.schema) {
      ...
      try {
        ...
        log.warn('Using fallback local schema as stitched schema was unavailable', {
          operation: 'createYoga.schema'
        });
      } catch (e) {
...
export async function startServer() {
  const REFRESH_INTERVAL = process.env.REFRESH_INTERVAL ? parseInt(process.env.REFRESH_INTERVAL, 10) : 30_000;
  ...
  Container.set('ServicePermissionRepository', dataSource.getRepository(ServicePermission));
  Container.set('PermissionTemplateRepository', dataSource.getRepository(PermissionTemplate));
  Container.set('UserServiceRoleRepository', dataSource.getRepository(UserServiceRole));
  ...
  try {
    ...
    log.info(`Loaded ${serviceEndpoints.length} services from database for schema polling`, {
      operation: 'loadServicesFromDatabase',
      metadata: {
        source: 'database',
  ...
  log.info(`Gateway schema auto-refresh started: polling every ${REFRESH_INTERVAL} ms`, {
